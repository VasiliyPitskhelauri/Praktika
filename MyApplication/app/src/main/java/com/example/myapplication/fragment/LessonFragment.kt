...
class LessonFragment : Fragment() {
    ...
    var database = FirebaseDatabase.getInstance().getReference("Lessons")
        ...
        var getdata = object : ValueEventListener {
            override fun onDataChange(p0: DataSnapshot) {
                for(i in p0.children){
                    ...
                    var amountHours = i.child("amountHours").getValue().toString().toInt()
                    var speciality = i.child("speciality").getValue().toString()
                    (activity as MainActivity?)?.addDataFive(id as Int,
                        title as String,
                        amountHours as Int,
